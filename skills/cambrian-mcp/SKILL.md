...
| `cambrian_solana_token_snapshot` | `token_address`, optional `token_symbol` | token details, current price, 24h/7d/30d price-volume, top holders, pools, Deep42 sentiment shifts |
| `cambrian_health` | none | up/down and latency per service (Solana, Base, Deep42, Risk) |
| `cambrian_usage` | none | rate-limit `limit`/`remaining`/`resetAt` per service |
...
| `RATE_LIMITED` | 429 | Retryable. Back off, then retry. `cambrian_usage` shows the reset time. |
